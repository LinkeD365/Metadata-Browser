...
export const MetadataBrowser = observer(
  (props: MetadataBrowserProps): React.JSX.Element => {
    ...
    async function getTableMeta() {
      ...
      await dvService
        ...
        .then((tables) => {
          ...
          onLog(
            `Loaded ${tables.length} tables from ${connection?.name}`,
            "success"
          );
        })
    ...
    const [selectedValue, setSelectedValue] =
      React.useState<TabValue>("tables");
    ...
    const handleRowDoubleClick = React.useCallback(
      (item: TableMeta) => {
        ...
        const exists = viewModel.selectedTables.some(
          (t) => t.tableName === item.tableName
        );
        if (!exists) {
    ...
    const tableDetails = viewModel.selectedTables?.map((t) => (
      <div
        key={t.tableName}
        role="tabpanel"
        aria-labelledby={`Table-${t.tableName}`}
      >
        <TableDetails
